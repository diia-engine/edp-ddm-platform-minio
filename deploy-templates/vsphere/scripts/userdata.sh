...
KES_DOWNLOAD_URL="${kes_download_url}"
...
MINIO_VOLUME_MOUNT_PATH="${minio_local_mount_path}"
...
MINIO_KES_API_KEY_FILE_PATH="${MINIO_KES_CLIENT_DIR}/minio.key"
...
function install_prerequisites() {
  ...
  curl --silent --output /usr/local/bin/mc "${MC_DOWNLOAD_URL}"
...
function install_kes_binary(){
  ...
  if [[ ! -f "${binary_path}" ]]; then
      wget -q "${KES_DOWNLOAD_URL}" -O ${binary_path}
      chmod +x ${binary_path}
...
function install_minio_binary() {
  ...
  wget -q "${MINIO_DOWNLOAD_URL}" -O ${binary_path}
  chmod +x ${binary_path}
...
chown ${MINIO_USER}:${MINIO_GROUP} ${MINIO_ENV_FILE}
  ...
  ExecStartPre=/bin/bash -c "if [ -z \"${MINIO_VOLUME_MOUNT_PATH}/\" ]; then echo \"Variable MINIO_VOLUMES not set in /etc/default/minio\"; exit 1; fi"
